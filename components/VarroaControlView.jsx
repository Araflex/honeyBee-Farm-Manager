...
const VarroaControlView = () => {
    ...
    const [beesCount, setBeesCount] = useState(100);
    const [mitesCount, setMitesCount] = useState(0);
    ...
    const getRiskLevel = (rate) => {
        ...
        if (rate < 3)
            return { color: 'text-yellow-600', label: 'Moderate Risk', bg: 'bg-yellow-50' };
        return { color: 'text-red-600', label: 'High Risk - Treat Now!', bg: 'bg-red-50' };
    };
    const handleSubmit = (e) => {
        e.preventDefault();
        if (!selectedApiaryId || !selectedHiveId)
            return;
        addLog({
            id: `l${Date.now()}`,
    ...
    return (<div className="p-6 space-y-6 animate-fade-in max-w-4xl mx-auto">
      <header>
      ...
      {showSuccess && (<div className="bg-green-100 border border-green-200 text-green-800 p-4 rounded-xl flex items-center gap-2 animate-bounce">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <select required className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-amber-500 transition-all outline-none" value={selectedApiaryId} onChange={e => { setSelectedApiaryId(e.target.value); setSelectedHiveId(''); }}>
              ...
              <div>
                ...
                <select required disabled={!selectedApiaryId} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-amber-500 transition-all outline-none disabled:opacity-50" value={selectedHiveId} onChange={e => setSelectedHiveId(e.target.value)}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative">
                   ...
                   <input type="number" min="1" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 pr-10 focus:ring-2 focus:ring-amber-500 outline-none" value={beesCount} onChange={e => setBeesCount(Number(e.target.value))}/>
                </div>
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">{t('varroa.mites_count')}</label>
                <input type="number" min="0" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-amber-500 outline-none" value={mitesCount} onChange={e => setMitesCount(Number(e.target.value))}/>
              </div>
            </div>

            <div>
              <label className="block text-sm font-bold text-slate-700 mb-2">{t('varroa.notes')}</label>
              <textarea className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 h-24 resize-none focus:ring-2 focus:ring-amber-500 outline-none" placeholder="..." value={notes} onChange={e => setNotes(e.target.value)}/>
            </div>

            <button type="submit" disabled={!selectedApiaryId || !selectedHiveId} className="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold py-4 rounded-xl shadow-lg shadow-amber-200 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:shadow-none">
