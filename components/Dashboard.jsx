            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                            <Pie data={hiveStatusData} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
                                {hiveStatusData.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color}/>))}
                            </Pie>
                            <Tooltip />
                            <Legend />
                        </PieChart>
             ...
             <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                ...
                <div className="space-y-4">
                    {recentActivity.map(log => (<div key={log.id} className="flex items-start gap-3 text-sm">
             ...
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-4">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                        {pendingLogs.length === 0 ? (<div className="p-8 text-center text-slate-500">{t('dash.no_tasks')}</div>) : (<div className="divide-y divide-slate-100">
                                {pendingLogs.map(log => {
                const apiary = apiaries.find(a => a.id === log.apiaryId);
                return (<div key={log.id} className="p-4 hover:bg-slate-50 transition-colors flex items-center justify-between group">
                                            <div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
                        <div className="space-y-2">
                            {apiaries.slice(0, 5).map(apiary => (<Link key={apiary.id} to={`/apiaries/${apiary.id}`} className="block p-3 rounded-lg hover:bg-amber-50 transition-colors border border-transparent hover:border-amber-100">
