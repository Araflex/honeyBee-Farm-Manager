...
import { LayoutDashboard, Map, ClipboardList, Bug, Menu, X, Users, LogOut, ShieldCheck, Microscope } from 'lucide-react';
    ...
    const SidebarContent = () => (<>
      ...
      <nav className="flex-1 p-4 space-y-1">
        {navItems.map(item => {
            ...
            return (<Link key={item.path} to={item.path} onClick={() => setIsSidebarOpen(false)} className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-all
      ...
      <div className="p-4 border-t border-amber-900/50 space-y-4">
          <div className="flex bg-amber-900/40 rounded-lg p-1">
             <button onClick={() => setLanguage('en')} className={`flex-1 text-xs font-bold py-1 rounded transition-colors ${language === 'en' ? 'bg-amber-700 text-white shadow' : 'text-amber-200 hover:text-white'}`}>
                EN
             </button>
             <button onClick={() => setLanguage('es')} className={`flex-1 text-xs font-bold py-1 rounded transition-colors ${language === 'es' ? 'bg-amber-700 text-white shadow' : 'text-amber-200 hover:text-white'}`}>
                ES
             </button>
          </div>
          ...
          <button onClick={handleLogout} className="w-full flex items-center gap-2 text-amber-200/60 hover:text-white hover:bg-red-900/30 px-4 py-2 rounded-lg transition-all text-sm">
