...
import { Plus, Box, ArrowLeft, Bot, Edit3, ClipboardList, Move, Crown, Calendar, Trash2, X, CheckCircle, Clock, Archive } from 'lucide-react';
import { LidType, NucleusStatus, HiveStatus, TaskType, TaskStatus, QueenStatus } from '../types';
...
const ApiaryDetail = () => {
    ...
    const [newNucleusStatus, setNewNucleusStatus] = useState(NucleusStatus.GOOD);
    ...
    const [draggedHiveId, setDraggedHiveId] = useState(null);
    if (!apiary)
        return <div className="p-6">Apiary not found</div>;
    // Drag and Drop Handlers
    const handleDragStart = (e, hiveId) => {
    ...
    const handleDrop = (e, targetPalletId, targetPosition) => {
        e.preventDefault();
        e.stopPropagation();
        if (!draggedHiveId)
            return;
    ...
    const handleMoveHiveToApiary = (e) => {
        e.preventDefault();
        if (!selectedHive || !promoteData.targetPalletId)
            return;
    ...
    const handleToggleTaskStatus = (log) => {
    ...
    const handleDeleteTask = (logId) => {
        if (window.confirm(t('detail.confirm_delete_task'))) {
            removeLog(logId);
        }
    };
    const openAddHiveModal = (palletId) => {
    ...
    const translateStatus = (status) => t(`status.${status.toLowerCase()}`);
    const promotionPallets = pallets.filter(p => {
        if (p.apiaryId !== promoteData.targetApiaryId)
            return false;
        const hiveCount = hives.filter(h => h.palletId === p.id && h.status !== HiveStatus.DEAD).length;
        return hiveCount < p.capacity;
    });
    return (<div className="p-4 md:p-6 space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex flex-wrap gap-3 w-full md:w-auto">
            <button onClick={handleAnalyze} disabled={isAnalyzing} className="flex-1 md:flex-none bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg shadow-indigo-100 transition-all active:scale-95 disabled:opacity-50">
                <Bot size={20}/> {isAnalyzing ? t('detail.analyzing') : t('detail.ai_check')}
            </button>
            <button onClick={() => setIsPalletModalOpen(true)} className="flex-1 md:flex-none bg-white border-2 border-slate-200 hover:border-amber-500 hover:text-amber-600 text-slate-700 px-5 py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 transition-all active:scale-95">
                <Box size={20}/> {t('detail.add_pallet')}
            </button>
        </div>
      </div>

      {aiAnalysis && (<div className="bg-indigo-50 border-2 border-indigo-100 p-6 rounded-2xl text-indigo-900 animate-fade-in shadow-sm relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
            ...
            return (<div key={pallet.id} onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, pallet.id)} className="bg-white border-2 border-slate-100 rounded-2xl p-5 relative group shadow-sm transition-all hover:border-amber-200 hover:shadow-md">
                            <div className="flex justify-between items-center mb-4">
                                <div>
                                    <span className="font-black text-slate-800 text-sm block tracking-wider">{pallet.code}</span>
                                    <div className="flex items-center gap-1.5 mt-1">
                                        <div className="w-16 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                            <div className="h-full bg-amber-500 transition-all duration-500" style={{ width: `${(hivesOnPallet.length / pallet.capacity) * 100}%` }}/>
                                        </div>
                                        <span className="text-[10px] font-bold text-slate-400">{hivesOnPallet.length}/{pallet.capacity}</span>
                                    </div>
                                </div>
                                <button onClick={() => openPalletTaskModal(pallet.id)} className="p-2 bg-amber-50 text-amber-600 rounded-lg hover:bg-amber-100 transition-colors" title={t('modal.pallet_task')}>
                    ...
                    return (<div key={`${pallet.id}-slot-${index}`} onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, pallet.id, index)} className="aspect-square">
                                            {hive ? (<div draggable onDragStart={(e) => handleDragStart(e, hive.id)} onClick={() => openEditHiveModal(hive)} className={`${getStatusColor(hive.status)} h-full border-2 p-3 rounded-2xl text-center cursor-move hover:scale-[1.02] active:scale-95 relative group/hive transition-all flex flex-col justify-center items-center shadow-sm`}>
                                                ...
                                                </div>) : (<button onClick={() => openAddHiveModal(pallet.id)} className="w-full h-full border-2 border-dashed border-slate-100 rounded-2xl flex items-center justify-center hover:bg-slate-50 hover:border-amber-200 group/add transition-colors" title={t('modal.add_hive')}>
            ...
            <div className="bg-white border-2 border-slate-100 rounded-2xl overflow-hidden shadow-sm">
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left border-collapse min-w-[450px]">
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {apiaryNuclei.length > 0 ? apiaryNuclei.map(nuc => (<tr key={nuc.id} className="hover:bg-slate-50 transition-colors group">
                ...
                nuc.status === NucleusStatus.BAD ? 'bg-red-100 border-red-200 text-red-700' : 'bg-blue-100 border-blue-200 text-blue-700'}`}>
                                    ...
                                    <td className="p-5 text-right">
                                        <button onClick={() => openEditNucleusModal(nuc)} className="p-2.5 bg-slate-50 text-slate-400 hover:bg-amber-100 hover:text-amber-700 rounded-xl transition-all shadow-sm">
                                            <Edit3 size={16}/>
                                        </button>
                                    </td>
                                </tr>)) : (<tr>
      ...
      <div className="space-y-6 pt-8 border-t-2 border-slate-100">
          <div className="flex items-center justify-between">
              ...
              <button onClick={() => openPalletTaskModal()} className="bg-amber-600 hover:bg-amber-700 text-white px-5 py-2.5 rounded-xl font-bold text-sm shadow-lg shadow-amber-100 transition-all flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            return (<div key={log.id} className={`p-5 rounded-2xl border-2 transition-all flex flex-col justify-between h-full ${isCompleted ? 'bg-slate-50 border-slate-100 opacity-60' : 'bg-white border-amber-50 shadow-sm hover:border-amber-300'}`}>
                          <div>
                          ...
                          <div className="flex items-center justify-between pt-4 border-t border-slate-100/50 mt-auto">
                              ...
                              <button onClick={() => handleToggleTaskStatus(log)} className={`flex items-center gap-1 text-xs font-bold px-3 py-1.5 rounded-lg transition-all ${isCompleted ? 'bg-amber-100 text-amber-700' : 'bg-green-600 text-white'}`}>
      ...
      {isPalletTaskModalOpen && (<div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 animate-fade-in p-4 backdrop-blur-sm">
                  ...
                  <form onSubmit={handleSavePalletTask} className="space-y-6">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl p-3 text-sm font-bold outline-none focus:ring-2 focus:ring-amber-500 transition-all" value={newTaskData.type} onChange={e => setNewTaskData({ ...newTaskData, type: e.target.value })}>
                      ...
                      <div>
                          ...
                          <div className="bg-slate-50 border-2 border-slate-100 rounded-xl max-h-48 overflow-y-auto p-3 grid grid-cols-2 gap-2 shadow-inner">
                              {hives.filter(h => h.palletId === selectedPalletId && h.status !== HiveStatus.DEAD).length > 0 ? (hives.filter(h => h.palletId === selectedPalletId && h.status !== HiveStatus.DEAD).map(h => (<label key={h.id} className="flex items-center gap-3 p-3 bg-white rounded-xl border-2 border-slate-50 cursor-pointer hover:border-amber-400 transition-all shadow-sm">
                                    ...
                                    </label>))) : (<div className="col-span-2 text-center py-4 text-slate-300 text-xs font-bold uppercase tracking-widest">
                                      Apiary Task (General)
                                  </div>)}
                          </div>
                      </div>
                      ...
                      <div>
                        ...
                        <textarea required className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl p-4 text-sm font-medium h-32 resize-none outline-none focus:ring-2 focus:ring-amber-500 transition-all" value={newTaskData.description} onChange={e => setNewTaskData({ ...newTaskData, description: e.target.value })} placeholder={t('modal.description_ph')}/>
                      </div>
      ...
      {isPalletModalOpen && (<div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 animate-fade-in p-4 backdrop-blur-sm">
      ...
      {/* Add/Edit Hive Modal */}
      {(isHiveModalOpen || isEditHiveModalOpen) && !isMoveHiveModalOpen && (<div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 animate-fade-in p-4 backdrop-blur-sm">
            <div className="bg-white p-6 rounded-2xl shadow-2xl max-md w-full my-8 max-h-[90vh] overflow-y-auto relative border border-slate-100">
                ...
                <div className="flex justify-between items-center mb-6">
                    ...
                    {isEditHiveModalOpen && (<button type="button" onClick={() => setIsMoveHiveModalOpen(true)} className="text-[10px] font-black uppercase tracking-widest bg-blue-50 text-blue-600 px-3 py-1.5 rounded-lg hover:bg-blue-100 flex items-center gap-1.5">
                ...
                <form onSubmit={isEditHiveModalOpen ? handleUpdateHive : handleAddHive} className="space-y-6">
                    <div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                            {Object.values(HiveStatus).map(status => (<button key={status} type="button" onClick={() => setNewHiveData({ ...newHiveData, status })} className={`text-[9px] py-3 rounded-xl border-2 transition-all font-black uppercase tracking-widest ${newHiveData.status === status
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl p-3 text-sm font-bold text-slate-700 outline-none" value={newHiveData.chamberCount} onChange={e => setNewHiveData({ ...newHiveData, chamberCount: parseInt(e.target.value) })}>
                        ...
                        <div>
                            ...
                            <select className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl p-3 text-sm font-bold text-slate-700 outline-none" value={newHiveData.lidType} onChange={e => setNewHiveData({ ...newHiveData, lidType: e.target.value })}>
                    ...
                    <div className="bg-purple-50 p-5 rounded-2xl border-2 border-purple-100 shadow-sm">
                        ...
                        <div className="space-y-4">
                             <div>
                                ...
                                <select className="w-full text-xs font-bold border-2 border-purple-100 rounded-xl p-3 bg-white outline-none focus:border-purple-300 transition-all" value={newHiveData.queenStatus} onChange={e => setNewHiveData({ ...newHiveData, queenStatus: e.target.value })}>
                                    {Object.values(QueenStatus).map(s => (<option key={s} value={s}>{t(`queen.${s.toLowerCase()}`)}</option>))}
                                </select>
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-[9px] font-black text-purple-400 mb-2 uppercase tracking-widest">{t('modal.queen_origin')}</label>
                                    <input type="text" className="w-full text-xs font-bold border-2 border-purple-100 rounded-xl p-3 bg-white outline-none focus:border-purple-300 transition-all" placeholder="..." value={newHiveData.queenOrigin} onChange={e => setNewHiveData({ ...newHiveData, queenOrigin: e.target.value })}/>
                                </div>
                                <div>
                                    <label className="block text-[9px] font-black text-purple-400 mb-2 uppercase tracking-widest">{t('modal.queen_date')}</label>
                                    <input type="date" className="w-full text-xs font-bold border-2 border-purple-100 rounded-xl p-3 bg-white outline-none focus:border-purple-300 transition-all" value={newHiveData.queenInstallDate} onChange={e => setNewHiveData({ ...newHiveData, queenInstallDate: e.target.value })}/>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col-reverse sm:flex-row justify-between items-center gap-4 mt-8 pt-6 border-t border-slate-50">
                        {isEditHiveModalOpen && (<button type="button" onClick={handleRemoveHive} className="w-full sm:w-auto text-red-500 hover:text-red-700 text-xs font-black uppercase tracking-widest flex items-center justify-center gap-2 px-4 py-2 rounded-xl hover:bg-red-50 transition-all">
                                 <Trash2 size={16}/> {t('modal.remove_hive')}
                             </button>)}
                        <div className="flex gap-3 w-full sm:w-auto ml-auto">
                            <button type="button" onClick={() => { setIsHiveModalOpen(false); setIsEditHiveModalOpen(false); }} className="flex-1 sm:flex-none px-6 py-2.5 text-slate-400 hover:text-slate-600 font-bold">
                                {t('modal.cancel')}
                            </button>
                            <button type="submit" className={`flex-1 sm:flex-none px-8 py-2.5 text-white rounded-xl font-black uppercase tracking-widest text-[10px] shadow-lg transition-all active:scale-95 ${isEditHiveModalOpen ? 'bg-blue-600 shadow-blue-100' : 'bg-amber-600 shadow-amber-100'}`}>
